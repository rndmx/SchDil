...
def CarsOnCenter : Prop :=
  ∀ (i : M.indnumb)
    ...
    ∃ g : (pull_loc_cov X M.Drep (M.Ysub i₀)
      (M.Ysub i₀ ↘ X) γβ.1).obj γβ.2,
      Ideal.map (pull_mor_ring X M.Drep (M.Ysub i₀)
          (M.Ysub i₀ ↘ X) γβ).hom
        (M.Dideal i γβ.1) = Ideal.span {g} ∧
      g ∈ nonZeroDivisors ((pull_loc_cov X M.Drep (M.Ysub i₀)
        (M.Ysub i₀ ↘ X) γβ.1).obj γβ.2)
...
def CarsOnY : Prop := M.CarsOnCenter i₀
