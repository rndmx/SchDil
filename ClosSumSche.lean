...
noncomputable def PreClos.prodIdeal (W Z : PreClos X)
    (e : W.indnumb ≃ Z.indnumb)
...
theorem ClosF.sum_comm {ι : Type} (W Z : ClosF X ι) : W.sum Z = Z.sum W := by
  ...
  simp only [Equiv.trans_apply, Equiv.cast_apply, cast_cast]
theorem PreClos.reindexIdeal_sum (Z V : PreClos X)
    (e : Z.indnumb ≃ V.indnumb)
    (cov' : Scheme.AffineCover.{u+1} (P := @IsOpenImmersion) X)
    (i : Z.indnumb) (γ : cov'.J) :
    (Z.sum V e).reindexIdeal cov' i γ =
      Z.reindexIdeal cov' i γ * V.reindexIdeal cov' (e i) γ := by
  have h := PreClos.ideal_eq_reindexIdeal
    ((Z.reindex cov').sum (V.reindex cov') e) (Z.sum V e)
    (PreClos.sumRel (Z.reindex cov') (V.reindex cov') Z V e e
      (Z.reindex_rel cov') (V.reindex_rel cov') (fun _ => rfl)) γ i
  have h3 : ((Z.reindex cov').sum (V.reindex cov') e).ideal i γ =
      Z.reindexIdeal cov' i γ * (V.reindex cov').reindexIdeal cov' (e i) γ := rfl
  have h4 : (V.reindex cov').reindexIdeal cov' (e i) γ =
      V.reindexIdeal cov' (e i) γ :=
    PreClos.reindexIdeal_self (V.reindex cov') (e i) γ
  rw [← h4]
  exact h.symm.trans h3

theorem PreClos.sum_assoc_ideal (W Z V : PreClos X)
    (e : W.indnumb ≃ Z.indnumb) (f : Z.indnumb ≃ V.indnumb)
    (g : W.indnumb ≃ V.indnumb) (hg : ∀ i, g i = f (e i))
    (i : W.indnumb) (γ : W.cov.J) :
    ((W.sum Z e).sum V g).ideal i γ =
      (W.sum (Z.sum V f) e).reindexIdeal W.cov i γ := by
  have hA : ((W.sum Z e).sum V g).ideal i γ =
      W.ideal i γ * Z.reindexIdeal W.cov (e i) γ *
        V.reindexIdeal W.cov (g i) γ := rfl
  have hB : (W.sum (Z.sum V f) e).reindexIdeal W.cov i γ =
      W.ideal i γ * (Z.sum V f).reindexIdeal W.cov (e i) γ :=
    PreClos.reindexIdeal_self (W.sum (Z.sum V f) e) i γ
  rw [hA, hB, PreClos.reindexIdeal_sum Z V f W.cov (e i) γ, mul_assoc, hg i]

noncomputable def PreClos.sum_assoc_rel (W Z V : PreClos X)
    (e : W.indnumb ≃ Z.indnumb) (f : Z.indnumb ≃ V.indnumb)
    (g : W.indnumb ≃ V.indnumb) (hg : ∀ i, g i = f (e i)) :
    relStructure ((W.sum Z e).sum V g) (W.sum (Z.sum V f) e) :=
  PreClos.relStructure_of_ideal_eq _ _ (Equiv.refl _)
    (fun i γ => PreClos.sum_assoc_ideal W Z V e f g hg i γ)

theorem ClosF.sum_assoc {ι : Type} (W Z V : ClosF X ι) :
    (W.sum Z).sum V = W.sum (Z.sum V) := by
  induction W using Quotient.inductionOn with | h W =>
  induction Z using Quotient.inductionOn with | h Z =>
  induction V using Quotient.inductionOn with | h V =>
  refine Quotient.sound ⟨⟨PreClos.sum_assoc_rel W.1 Z.1 V.1 _ _ _ ?_, ?_⟩⟩
  · intro i
    simp only [Equiv.trans_apply, Equiv.cast_apply, cast_cast]
  · intro i
    rfl

noncomputable def zeroChartDatum
    (cov : Scheme.AffineCover.{u+1} (P := @IsOpenImmersion) X) : ChartDatum X where
  cov := cov
  idl _ := ⊤
  compat _ _ _ := by rw [Ideal.map_top, Ideal.map_top]

noncomputable def PreClos.zero (ι : Type)
    (cov : Scheme.AffineCover.{u+1} (P := @IsOpenImmersion) X) : PreClos X where
  indnumb := ι
  subscheme _ := (zeroChartDatum cov).glued
  over _ := ⟨(zeroChartDatum cov).structureMap⟩
  cov := cov
  ideal _ _ := ⊤
  condiso _ γ := asIso ((zeroChartDatum cov).chartCompare γ)
  condover _ γ := by
    rw [Scheme.Hom.isOver_iff, pullback_over_right,
      spec_quotient_ideal_over_eq]
    show (zeroChartDatum cov).chartCompare γ ≫
        pullback.snd ((zeroChartDatum cov).structureMap) (cov.map γ) =
      Spec.map (CommRingCat.ofHom (Ideal.Quotient.mk (⊤ : Ideal (cov.obj γ))))
    exact (zeroChartDatum cov).chartCompare_snd γ

noncomputable def zeroGluedIso
    (cov cov' : Scheme.AffineCover.{u+1} (P := @IsOpenImmersion) X) :
    (zeroChartDatum cov).glued ≅ (zeroChartDatum cov').glued :=
  ((zeroChartDatum cov).existsUnique_iso_glued_of_idl_map_eq (zeroChartDatum cov')
    (fun _ _ _ => by
      show Ideal.map _ (⊤ : Ideal _) = Ideal.map _ (⊤ : Ideal _)
      rw [Ideal.map_top, Ideal.map_top])).choose

theorem zeroGluedIso_over
    (cov cov' : Scheme.AffineCover.{u+1} (P := @IsOpenImmersion) X) :
    (zeroGluedIso cov cov').hom ≫ (zeroChartDatum cov').structureMap =
      (zeroChartDatum cov).structureMap :=
  ((zeroChartDatum cov).existsUnique_iso_glued_of_idl_map_eq (zeroChartDatum cov')
    (fun _ _ _ => by
      show Ideal.map _ (⊤ : Ideal _) = Ideal.map _ (⊤ : Ideal _)
      rw [Ideal.map_top, Ideal.map_top])).choose_spec.1

noncomputable def PreClos.zeroRel (ι : Type)
    (cov cov' : Scheme.AffineCover.{u+1} (P := @IsOpenImmersion) X) :
    relStructure (PreClos.zero ι cov) (PreClos.zero ι cov') where
  indnumb_equiv := Equiv.refl ι
  subscheme_iso _ := zeroGluedIso cov cov'
  subscheme_iso_over _ := by
    rw [Scheme.Hom.isOver_iff]
    exact zeroGluedIso_over cov cov'

theorem PreClos.zero_reindexIdeal (ι : Type)
    (cov cov' : Scheme.AffineCover.{u+1} (P := @IsOpenImmersion) X)
    (i : ι) (γ : cov'.J) :
    (PreClos.zero ι cov).reindexIdeal cov' i γ = ⊤ :=
  (PreClos.ideal_eq_reindexIdeal (PreClos.zero ι cov') (PreClos.zero ι cov)
    (PreClos.zeroRel ι cov' cov) γ i).symm

theorem PreClos.sum_zero_ideal (W : PreClos X) (ι : Type)
    (cov : Scheme.AffineCover.{u+1} (P := @IsOpenImmersion) X)
    (e : W.indnumb ≃ ι) (i : W.indnumb) (γ : W.cov.J) :
    (W.sum (PreClos.zero ι cov) e).ideal i γ = W.reindexIdeal W.cov i γ := by
  have hA : (W.sum (PreClos.zero ι cov) e).ideal i γ =
      W.ideal i γ * (PreClos.zero ι cov).reindexIdeal W.cov (e i) γ := rfl
  rw [hA, PreClos.zero_reindexIdeal ι cov W.cov (e i) γ, Ideal.mul_top,
    PreClos.reindexIdeal_self W i γ]

noncomputable def PreClos.sum_zero_rel (W : PreClos X) (ι : Type)
    (cov : Scheme.AffineCover.{u+1} (P := @IsOpenImmersion) X)
    (e : W.indnumb ≃ ι) :
    relStructure (W.sum (PreClos.zero ι cov) e) W :=
  PreClos.relStructure_of_ideal_eq _ _ (Equiv.refl _)
    (fun i γ => PreClos.sum_zero_ideal W ι cov e i γ)

noncomputable def ClosF.zero (X : Scheme.{u+1}) (ι : Type) : ClosF X ι :=
  Quotient.mk'' ⟨PreClos.zero ι (Scheme.affineOpenCover X), rfl⟩

theorem ClosF.sum_zero {ι : Type} (W : ClosF X ι) :
    W.sum (ClosF.zero X ι) = W := by
  induction W using Quotient.inductionOn with | h W =>
  exact Quotient.sound ⟨⟨PreClos.sum_zero_rel W.1 ι _ _, fun _ => rfl⟩⟩

noncomputable instance ClosF.instAdd {ι : Type} : Add (ClosF X ι) :=
  ⟨ClosF.sum⟩

noncomputable instance ClosF.instZero {ι : Type} : Zero (ClosF X ι) :=
  ⟨ClosF.zero X ι⟩

noncomputable instance ClosF.instAddCommMonoid {ι : Type} :
    AddCommMonoid (ClosF X ι) where
  add_assoc := ClosF.sum_assoc
  zero_add W := (ClosF.sum_comm _ W).trans (ClosF.sum_zero W)
  add_zero := ClosF.sum_zero
  add_comm := ClosF.sum_comm
  nsmul := nsmulRec
  nsmul_zero _ := rfl
  nsmul_succ _ _ := rfl

theorem ClosF.add_eq_sum {ι : Type} (W Z : ClosF X ι) : W + Z = W.sum Z := rfl

theorem ClosF.zero_eq {ι : Type} : (0 : ClosF X ι) = ClosF.zero X ι := rfl
